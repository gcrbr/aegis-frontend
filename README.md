...
The HTML/JavaScript application is served by Nginx on port 80.
Requests to `/api/` are proxied to `http://backend:5000`, preserving the existing
Compose and Kubernetes service names. The browser uses the same origin for UI and API.

`index.html`, `nginx.conf` and `Dockerfile` live at the repository root.
...
```bash
docker build -t gcrbr/frontend:local .
```
...
image as `gcrbr/frontend:<commit-sha>` and `:latest`, then update only
`k8s/09-frontend-deployment.yaml` in `gcrbr/aegis-app-manifests`.

Set these repository Actions secrets:

| Secret | Value |
| --- | --- |
| `DOCKER_USERNAME` | Docker Hub account with write access to `gcrbr/frontend` |
...
Actions variable `RELEASE_ENABLED` to `true`, then run the `Aegis Frontend`
...
Git history was filtered from the original `frontend/` directory, retaining
